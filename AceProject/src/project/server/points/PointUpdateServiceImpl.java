...
public class PointUpdateServiceImpl extends RemoteServiceServlet implements
		PointUpdateService {
	@Override
	public List<String> updatedList() {
		...
		DatastoreService datastore = DatastoreServiceFactory
				.getDatastoreService();
		Query q = new Query("AceProjectUser").addSort("points",
				Query.SortDirection.DESCENDING); // used for leaderboard
		List<Entity> allUsers = datastore.prepare(q).asList(
				FetchOptions.Builder.withLimit(5)); // used for leaderboard
		...
		for (int x = 0; x < 10; x++) {
			...
			long b = (Long) a;
			strings.add(Long.toString(b) + "  "
					+ (String) e.getProperty("nickname")); // This is what is
															// displayed in the
															// right column
		}
		return strings;
