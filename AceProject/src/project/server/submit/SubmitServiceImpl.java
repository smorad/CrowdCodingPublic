...
public class SubmitServiceImpl extends RemoteServiceServlet implements
		SubmitService {
	// loggers must be used since println doesn't work serverside
	private final Logger logger = Logger.getLogger("NameOfYourLogger"); // debug
		...
		while (true) {
			try {
				...
				if (o.getTxn().isActive()) {
					o.getTxn().rollback();
					logger.log(Level.SEVERE, "Pull " + (count++)
							+ " failed rolling back");
				}
			}
		}
	...
	private PersistObject chooseRandomly() { // randomly chooses microtask
		...
		Query<PersistObject> q = o.query(PersistObject.class)
				.filter("isDone", false).filter("checkedOut", false);
	...
	private UserStoryInfo transferToClient(UserStoryPersist pInfo) {
		// transfer to client just moves all the info from a persist class to an
		// info class, so it can be served to client
	...
	@Override
	public void submit(InfoObject info) { // on submit, transfers information
	...
	private void transferToServer(Objectify o, UserStoryInfo info) {
		...
		Key k = new Key<UserStoryPersist>(UserStoryPersist.class,
				info.getKeyString());
	...
	private void transferToServer(Objectify o, EntryPointInfo info) {
		@SuppressWarnings("rawtypes")
		Key k = new Key<EntryPointPersist>(EntryPointPersist.class,
				info.getKeyString());
	...
	private void transferToServer(Objectify o, TestCaseInfo info) {
		@SuppressWarnings("rawtypes")
		Key k = new Key<TestCasePersist>(TestCasePersist.class,
				info.getKeyString());
	...
	private void transferToServer(Objectify o, AceEditorInfo info) {
		@SuppressWarnings("rawtypes")
		Key k = new Key<AceEditorPersist>(AceEditorPersist.class,
				info.getKeyString());
		...
		logger.log(
				Level.SEVERE,
				""
						+ o.get(new Key<AceEditorPersist>(
								AceEditorPersist.class, pInfo.getId()))
								.getCode());
	...
	private void transferToServer(Objectify o, UnitTestInfo info) {
		@SuppressWarnings("rawtypes")
		Key k = new Key<UnitTestPersist>(UnitTestPersist.class,
				info.getKeyString());
