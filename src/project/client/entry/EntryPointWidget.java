...
public class EntryPointWidget extends ScreenWidget {
	...
	public void UI() {
		...
		instructions.add(h);
		text = new TextArea(); // this should probably be changed to HTML in the
								// future
		text.setText(story);
		text.setReadOnly(true);
		text.setSize("600px", "80px");
		text.setStyleName("dialogVPanel"); // makes the font black instead of
											// grey
		DOM.setStyleAttribute(text.getElement(), "border", "1px"); // removes
																	// border
		DOM.setStyleAttribute(text.getElement(), "minHeight", "100px");
		DOM.setStyleAttribute(text.getElement(), "width", "600px"); // fixes
																	// size
																	// error on
																	// firefox
	...
	public void submit() {
		...
		for (int x = 0; x < numTabs; x++) { // gets info from entrypointtab and
											// saves it in einfo object
			...
			e.setReturnType(info.getReturnType());
			e.addTest();
			eInfo.addMethod(e);
		}
		eInfo.setDone(true);
